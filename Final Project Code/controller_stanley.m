function [log_error, log_z, log_torque, log_Md] = controller_stanley(w, z)
...
global Mc_desired Md Fy_total Fxd m Mc;
m = 6762; % m in kg
...
a_lateral = 0; % Lateral acceleration
...
Kvp = 10; % Proportional Control gain for speed 
Kvi = 0; % Integral Control gain for speed
Kphi_p = 20; % Proportional Control gain for yaw
Kphi_d = 0; % Derivative Control gain for yaw 
...
for t=1:ts:t_final
   ...
   if i==n
    ...
    v_robot = [z(1) - w1(1); z(2) - w1(2); 0];
    v_waypoint = [w2(1) - w1(1); w2(2) - w1(2); 0];

    vr_way_cross = cross(v_robot, v_waypoint);
    error_lateral = sqrt((xd-z(1))^2 + (yd-z(2))^2)*sign(vr_way_cross(3));

    if abs(error_lateral)>20
        break;
    end
    ...
    error_yaw_ip = error_yaw + atan2(2*error_lateral, abs(z(3))+2);
    ...
    Mc = (Kphi_d*error_yaw_rate + Kphi_p*error_yaw_ip)*Iz ;
